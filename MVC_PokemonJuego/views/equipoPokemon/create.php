...
$pokemons = $controlador->listarTodos($_SESSION["usuario"]->id);
$visibilidad = "hidden";
?>
<div id="contenedor_main">
    ...
    <main id="contenedor_listar">
        <div>
            <h1 class="titulo">Tu Equipo Pokemon</h1>
        </div>

        <!--FORMULARIO PARA ELEGIR LOS POKEMONS DEL EQUIPO-->
        <div>

            <form action="index.php?tabla=equipoPokemon&accion=guardar" method="post">

                <div>
                    <label for="idPokemonEvolucion">Pokemon 1</label>
                    <select id="idPokemonEvolucion" name="idPokemonEvolucion">
                        <option value="">---- Elije el Pokemon 1 de tu Equipo ----</option>
                        <?php
                ...
                <div>
                    <label for="idPokemonEvolucion">Pokemon 2</label>
                    <select id="idPokemonEvolucion" name="idPokemonEvolucion">
                        <option value="">---- Elije el Pokemon 2 de tu Equipo ----</option>
                        <?php
                        foreach ($pokemons as $pokemon):
                            echo "<option value='{$pokemon->id}'> id: {$pokemon->id} - {$pokemon->nombre} - {$pokemon->tipo} - {$pokemon->nivel}</option>";
                        endforeach; ?>
                ...
                <div>
                    <label for="idPokemonEvolucion">Pokemon 3</label>
                    <select id="idPokemonEvolucion" name="idPokemonEvolucion">
                        <option value="">---- Elije el Pokemon 3 de tu Equipo ----</option>
                        <?php
                        foreach ($pokemons as $pokemon):
                            echo "<option value='{$pokemon->id}'> id: {$pokemon->id} - {$pokemon->nombre} - {$pokemon->tipo} - {$pokemon->nivel}</option>";
                        endforeach; ?>
        ...
        <div>
            <h1 class="titulo">Tus Pokemon</h1>
        </div>
